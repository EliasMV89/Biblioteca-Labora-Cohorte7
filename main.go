...
func main() {
	...
	for {
		...
		case 1:
			...
			nuevoLibro := servicios.Libro{
				Titulo:           titulo,
				ID_Autor:         id_autor,
				Anio_Publicacion: anio_publicacion,
				Disponible:       1,
			}
			servicios.AgregarLibro(db, nuevoLibro)
		case 2:
			fmt.Println("Agregar un nuevo autor")
			fmt.Print("Ingrese el nombre del autor: ")
			var nombre string
			fmt.Scanln(&nombre)
			nuevoAutor := servicios.Autor{
				Nombre: nombre,
			}
			servicios.AgregarAutor(db, nuevoAutor)
		...
		case 3:
			...
			var telefono string
			fmt.Scanln(&telefono)
			nuevoUsuario := servicios.Usuario{
				Nombre:   nombre,
				Apellido: apellido,
				Email:    email,
				Telefono: telefono,
			}
			servicios.AgregarUsuario(db, nuevoUsuario)
		case 4:
			...
			nuevoPrestamo := servicios.Prestamo{
				ID_Libro:       id_libro,
				ID_Usuario:     id_usuario,
				Fecha_Prestamo: fecha_Prestamo,
			}
			servicios.PrestarLibro(db, nuevoPrestamo)
